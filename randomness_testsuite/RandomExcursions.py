...
class RandomExcursions:
    ...
    def random_excursions_test(binary_data: str, verbose=False, state=1):
        ...
        pi = [
            ([RandomExcursions.get_pi_value(uu, state) for uu in range(6)])
        ...
        for item in p_values:
            result.append(
                (states[count], x_values[count], xObs[count], item, (item >= 0.01))
            )
            count += 1
    ...
    def variant_test(binary_data: str, verbose=False):
        ...
        li_data = []
        index = []
        for count in sorted([float(x) for x in set(cumulative_sum)]):
            if abs(count) <= 9:
                index.append(count)
        ...
        for count in sorted(set(index)):
            if not count == 0:
                ...
                p_values.append(
                    erfc(abs(RandomExcursions.get_frequency(li_data, count) - j) / den)
                )
